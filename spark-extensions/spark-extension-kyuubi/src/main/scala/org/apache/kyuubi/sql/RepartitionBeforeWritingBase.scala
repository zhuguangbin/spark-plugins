...
/**
 * For datasource v2 table, there two commands can write data to table
 * 1. AppendData: insert into xxx_v2_external_table as select XXX
 * 2. OverwriteByExpression: insert overwrite xxx_v2_external_table as select XXX
 * This rule add a repartition node between write and query
 */
abstract class RepartitionBeforeWritingDatasourceV2Base extends RepartitionBuilder {
  ...
  private def isInsertIntoDelegateV2(table: DataSourceV2Relation, query: LogicalPlan): Boolean =
    query.resolved && table.table.isInstanceOf[V1Table] && table.table.asInstanceOf[V1Table].catalogTable.bucketSpec.isEmpty && canInsertRepartitionByExpression(query)

  private def addRepartition(plan: LogicalPlan): LogicalPlan = plan match {
    case c @ CreateTableAsSelect(_, _, partitioning, query, _, _, _) if query.resolved && partitioning.isEmpty && canInsertRepartitionByExpression(query) =>
      c.copy(query = buildRepartition(Seq[Attribute](), query))

    case a @ AppendData(table: DataSourceV2Relation, query, _, _) if isInsertIntoDelegateV2(table, query) =>
      val delegate = table.table.asInstanceOf[V1Table].catalogTable
      val dynamicPartitionColumns =
        query.output.filter(attr => delegate.partitionColumnNames.contains(attr.name))
      a.copy(query = buildRepartition(dynamicPartitionColumns, query))

    case o @ OverwriteByExpression(table: DataSourceV2Relation, _, query, _, _) if isInsertIntoDelegateV2(table, query) =>
      val delegate = table.table.asInstanceOf[V1Table].catalogTable
      val dynamicPartitionColumns =
        query.output.filter(attr => delegate.partitionColumnNames.contains(attr.name))
      o.copy(query = buildRepartition(dynamicPartitionColumns, query))

    case oo @ OverwritePartitionsDynamic(table: DataSourceV2Relation, query, _, _) if isInsertIntoDelegateV2(table, query) =>
      val delegate = table.table.asInstanceOf[V1Table].catalogTable
      val dynamicPartitionColumns =
        query.output.filter(attr => delegate.partitionColumnNames.contains(attr.name))
      oo.copy(query = buildRepartition(dynamicPartitionColumns, query))
