...
class AddonSparkSqlParser(session: SparkSession, delegate: ParserInterface) extends ParserInterface with Logging {
  private lazy val builder = new AddonSparkSqlAstBuilder(session, delegate)
  ...
  protected def parse[T](command: String)(toResult: AddonSqlBaseParser => T): T = {
    logDebug(s"Parsing command: $command")

    val lexer = new AddonSqlBaseLexer(new UpperCaseCharStream(CharStreams.fromString(command)))
    lexer.removeErrorListeners()
    lexer.addErrorListener(ParseErrorListener)

    val tokenStream = new CommonTokenStream(lexer)
    val parser = new AddonSqlBaseParser(tokenStream)
