...
import cn.com.bigdata123.spark.plugin.catalog.hive.write.HiveFileBatchWrite
...
abstract class ProviderFileWriteBuilder(
  ...
  private def validateInputs(caseSensitiveAnalysis: Boolean): Unit = {
    ...
    InternalBridge.schemaUtils.checkColumnNameDuplication(schema.fields.map(_.name),
